...
/*   Updated: 2024/08/24 17:54:22 by rscherl          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	exit_with_cleanup(char *cmd, t_data *data, char **paths, int exit_code)
{
    if (cmd)
    {
        ft_putstr_fd(cmd, 2);
...
static void search_and_exec(char *cmd, t_elst *env_lst, t_cmd *cmd_struct, t_data *data)
{
    char	**paths;
	char	*env_paths;
	char	*path;
    ...
    if (!env_paths) exit_with_cleanup(cmd, data, NULL, 127);
